...
\begin{align}
w'_{\text{target}} &= \min(0.95,\; w_{\text{target}} + \alpha)
\end{align}

\textbf{Dampen ($\delta_{\text{item}}$)}:
\begin{align}
w'_{\text{target}} &= \max(0.05,\; w_{\text{target}} - \alpha)
\end{align}
...
After either operation, let $S=\sum_{j \neq \text{target}} w_j$ and $r = 1-w'_{\text{target}}$. If $S>0$, non-target items are redistributed proportionally:
\begin{align}
w'_i = \frac{w_i}{S}\,r \quad \forall i \neq \text{target}
\end{align}
In the evaluated scenarios, $S>0$ always holds, so $\sum_i w'_i = 1$ after each update.
...
The reference implementation additionally accepts Unicode operator symbols (e.g., $\sigma$, $\delta$) as parser aliases.
...
\begin{enumerate}
    ...
    \item For each task, initialize state from scenario-defined initial weights
