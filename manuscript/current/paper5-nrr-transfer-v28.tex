...
The two operators are designed to satisfy operator design principles (NRR-Phi~\cite{saito2026phi}, Appendix~D) through 
proportional weight redistribution~\cite{saito2026ime}:
\begin{itemize}
    \item $\sigma$ (\textit{strengthen}): Increase target weight by redistributing weight from other items proportionally\footnote{These operators ($\sigma$, $\delta$) act on individual items within a state, implementing the relative structure preservation principle from NRR-Phi (\cite{saito2026phi}, Appendix~D) through proportional redistribution. This differs from state-level components in NRR-Phi (e.g., $\sigma_{\mathrm{state}}$ calibration and $\delta$ v2 dampening), which transform entire weight distributions globally. Both preserve non-collapse through proportional relationships, but at different levels of abstraction.}
    \item $\delta$ (\textit{dampen}): Decrease target weight by redistributing it to other items proportionally
\end{itemize}

\noindent\textit{Note:} These operators act on individual items, implementing 
NRR's relative structure preservation principle~(\cite{saito2026phi}, Appendix~D) via proportional 
redistribution. This differs from the global state-level operators introduced 
...
critical for non-collapse. By maintaining proportional relationships during 
weight updates, they avoid the uniform subtraction shown to cause 
up to 17.8\% collapse rate \cite{saito2026phi}. The difference is one of abstraction level: NRR-Phi Appendix~D 
...
\begin{align}
w_{\text{target}}' &= w_{\text{target}} + \alpha \cdot \sum_{i \neq \text{target}} w_i \\
w_i' &= w_i \cdot (1 - \alpha) \quad \forall i \neq \text{target}
\end{align}

\textbf{Dampen ($\delta_{\text{item}}$)}:
\begin{align}
w_{\text{target}}' &= w_{\text{target}} \cdot (1 - \alpha) \\
w_i' &= w_i + \frac{\alpha \cdot w_{\text{target}}}{n-1} \quad \forall i \neq \text{target}
\end{align}

where $\alpha = 0.8$\footnote{Chosen empirically for stability across all six domains. Larger values (e.g., 0.9) risk premature collapse; smaller values (e.g., 0.5) provide insufficient signal. Domain-specific tuning could optimize this parameter further. Note that this $\alpha$ parameterizes proportional redistribution across $n$ items and is not directly comparable to the additive step size $\alpha = 0.4$ in NRR-IME~\cite{saito2026ime}, which uses absolute increments sufficient for its two-interpretation scenarios.} (redistribution factor) and $n$ is the number of items. These are item-level operators; later result sections use the shorthand symbols $\sigma, \delta$.

Weights are normalized after each operation to maintain $\sum w_i = 1$.
...
simplification that preserves non-collapse through proportional redistribution.
...
\begin{itemize}
    \item Security breach (4 turns): Threat assessment across expert domains
    ...
    \item Technical debt (3 turns): Performance issues, refactoring considerations
\end{itemize}
...
\begin{itemize}
    ...
    \item Target audience (4 turns): Demographics shifts across age groups
    \item Competitive analysis (3 turns): Competitor moves, differentiation needs
...
\begin{itemize}
    ...
    \item Max tokens: 1000 (ample for operator extraction)
...
\begin{itemize}
    ...
    \item Operator extraction success (binary: correct format and valid target)
    \item State evolution tracking (weight history)
\end{itemize}

\subsubsection{Extraction Logic}

We use simple pattern matching to extract operators:
...
\textbf{Success criterion}: Both operator and target must be extracted and target must be a valid item label.
...
\begin{enumerate}
    \item \textbf{Constrained output format}: Two-line structure (\texttt{operator:}, \texttt{target:}) with minimal ambiguity
...
We release code, fixed protocol settings, and primary logs for the cross-domain validation package.
